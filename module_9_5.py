...
class Iterator:
    ...
    def __init__(self, start, stop, step=1):  # Создаем объект итератора
        if step == 0:  # Проверка на нулевой шаг
            raise StepValueError('Шаг не может быть равен 0')
        self.start = start
        self.stop = stop
        self.step = step
        self.pointer = start  # Изначальное значение указателя

    def __iter__(self):
        self.pointer = self.start  # Сбрасываем указатель на старт
        return self
